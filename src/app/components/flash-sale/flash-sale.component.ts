...
export class FlashSaleComponent extends AppComponentBase implements OnInit {
  ...
  getFlashSale(): void {
    this.productService.productFlashSale().subscribe(rs => {
  ...
  getFlashSaleFuture(){
    this.productService.productFlashSaleFuture().subscribe(rs => {
      this.productFlashSaleFuture = rs.data;
    })
  }
